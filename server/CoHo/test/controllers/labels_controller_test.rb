...
class LabelsControllerTest < ActionController::TestCase
  ...
  test "should create label" do
    assert_difference('Label.count') do
      post :create, label: { name: @label.name, question_id: @label.question_id, user_id: @label.user_id }
    end
  ...
  test "should update label" do
    patch :update, id: @label, label: { name: @label.name, question_id: @label.question_id, user_id: @label.user_id }
    assert_redirected_to label_path(assigns(:label))
  end
